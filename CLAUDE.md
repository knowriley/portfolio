...
`WorkGrid` renders the (non-hidden) studies as a **2-column grid** (`grid-cols-1 md:grid-cols-2`) of `CaseStudyCard`s; each card is **thumbnail + title + date only** — the `description` field is *not* shown on the card. `CaseStudyCard` is **thumbnail-first by default** (used by `WorkGrid`); pass the `textOnTop` prop to render **title + date above the thumbnail** — this is how the case-study **"Up Next"** sections render it (`<CaseStudyCard study={nextStudy} textOnTop />`). The standard Up Next block is `<SectionDivider /> + <Label>Next</Label> + <CaseStudyCard study={nextStudy} textOnTop />` where `nextStudy = caseStudies.find((s) => !s.hidden && s.slug !== '<this-slug>')`. Instead, every case-study page must export full preview metadata derived from its entry: `{ title, description, openGraph: { title, description, type: 'article', url: '/work/<slug>' } }`. The social image is supplied by the Next.js **file convention** — drop a `1200×630` `opengraph-image.jpg` in the route folder (`app/work/[slug]/opengraph-image.jpg`); it's auto-wired as `og:image`/`twitter:image`, so don't declare images in the metadata object. A new case study isn't "done" until it has both the metadata export and the OG image.
